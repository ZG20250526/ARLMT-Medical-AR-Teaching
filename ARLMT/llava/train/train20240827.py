...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="/data0/GYF/offline_model/facebook/opt-125m")
...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            ...
            replace_token = DEFAULT_IMAGE_TOKEN * image_token_len
...
def preprocess_plain(
    ...
    for source in sources:
        try:
            ...
            logging.error(f"Assertion error for source: {source}. Error: {e}")
...
def preprocess(
    ...
    #     except AssertionError as e:
    #         logging.error(f"Assertion error in preprocess_plain. Error: {e}")
    # if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
    #     return preprocess_llama_2(sources, tokenizer, has_image=has_image)
    # if conversation_lib.default_conversation.version.startswith("v1"):
    #     #return preprocess_v1(sources, tokenizer, has_image=has_image)
    #     return preprocess_v1(sources, tokenizer) #llava-med
    ...
    conversations_tokenized = _tokenize_fn(conversations, tokenizer)    #llava-med
    ...
    input_ids = conversations_tokenized["input_ids"]    #llava-med
    targets = copy.deepcopy(input_ids)
    for target, source in zip(targets, sources):
        # if has_image:
        ...
        tokenized_lens = _tokenize_fn([header] + [s["value"] for s in source], tokenizer)["input_ids_lens"]    #llava-med
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        source = [example["conversations"] for example in list_data_dict]
        data_dict = preprocess(source, tokenizer)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            try:
                ...
                expected_size = (336, 336)
                if image.size != expected_size:
                    logging.warning(f"Image size {image.size} does not match expected size {expected_size}")
                    image = image.resize(expected_size, Image.Resampling.LANCZOS)
                    image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
                    print("Changed Image Size: ", image.size())
            except Exception as exn:
                logging.error(f"Error processing image {image_file}: {exn}")
            ...
            print('image-token-len: ',image_token_len)
            #patch_size = int(image.shape[1]//math.sqrt(image_token_len))
            if image_token_len <= 0:
                logging.error(f"image_token_len is zero or negative, get value:{image_token_len} with image shape : {image.shape}")
                raise ValueError("image_token_len should be greater than 0")
            patch_size = int(image.shape[1] / math.sqrt(image_token_len))
            print('patch_size: ',patch_size)
            cur_token_len = (image.shape[1]//patch_size) * (image.shape[2]//patch_size) #FIXME: 14 s hardcoded patch size
        ...
        print("data_dict labels :", len(data_dict["labels"]))
        if isinstance(i, int):
            data_dict = dict(input_ids = data_dict["input_ids"][0],
                             labels = data_dict["labels"][0])
